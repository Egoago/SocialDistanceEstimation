...
class YoloV4(Detector):
    ...
    def detect(self, image: np.array) -> List[BoundingBox]:
        ...
        original_image = cv2.imread("files/img.png")
        ...
        cv2.imshow('im', np.asarray(original_image))
        cv2.waitKey(0)

        # Skipping Step 2 (optional): Save the preprocessed image in TensorProto format
        ...
        sess = rt.InferenceSession("files/model.onnx")
        ...
        # Skipping Step 4 (optional): Save the preprocessed image in TensorProto format
        ...
        def draw_bbox(image, bboxes, classes=read_class_names("files/coco.names"), show_label=True):
            """
        ...
        ANCHORS = "files/yolov4_anchors.txt"
        STRIDES = [8, 16, 32]
        ...
        # Added: Convert and return
        def convert_bbox(bbox: list) -> BoundingBox:
            raise NotImplementedError

        def is_person(bbox: list) -> bool:
            raise NotImplementedError

        bounding_boxes = []
        for bbox in bboxes:
            if is_person(bbox):
                bounding_boxes.append(convert_bbox(bbox))
